(ns sprayshop.actor
  "SprayShopActor — the ISCO-08 7132 spray-shop scheduling/logistics
  coordination actor as a `langgraph.graph/state-graph`
  (ADR-2607121000 / CLAUDE.md Actors section). One graph run = one
  shop-coordination operation request (intake -> advise -> govern ->
  ...
  cloud-itonami-isco-7131's paintcrew.actor (the closely-related
  coating-application safety pattern).
  ...
  The unconditional invariant: the Spray Shop Advisor can never
  directly commit an operating record the SprayShopGovernor refuses,
  and can never make this actor finalize a spray-application-execution
  decision (e.g. a specific spray-coating or spray-finish application)
  or override a shop safety officer's judgment — every commit-record!
  ...
  spray-painting work or authorizes spray-application-execution
  decisions itself."
...
(defn build-graph
  "Build a compiled SprayShopActor graph. `store` implements
  `sprayshop.store/Store`. `advisor` implements
  `sprayshop.advisor/Advisor` (defaults to `mock-advisor`).
  ...
  (-> (g/state-graph
      ...
      (g/add-node :commit
                   (fn [{:keys [request proposal]}]
                     (let [record {:sprayer-id (:sprayer-id request)
                                    :op (:op proposal)
