(ns sprayshop.governor-test
  (:require [clojure.test :refer [deftest is testing]]
            [sprayshop.store :as store]
            [sprayshop.advisor :as advisor]
            [sprayshop.governor :as governor]))
...
(defn- op [op-kw & {:as extra}]
  (merge {:op op-kw :effect :propose :shop-id "SH-1"
          :confidence 0.9 :stake :low}
         extra))

(def ^:private req {:sprayer-id "sprayer-1"})
...
(deftest hard-on-unregistered-sprayer
  (let [st (fresh-store)
        v (governor/check {:sprayer-id "nobody"} {} (op :log-work-record) st)]
    (is (:hard? v))
    (is (some #(= :no-sprayer (:rule %)) (:violations v)))))

(deftest hard-on-unregistered-shop
  (let [st (fresh-store)
        v (governor/check req {} (op :log-work-record :shop-id "SH-ghost") st)]
    (is (:hard? v))
...
(deftest hard-on-scope-excluded-op-finalize-spray-application-decision
  (testing "finalizing a spray-application-execution decision is a permanent block, never a routine op"
    (let [st (fresh-store)
          v (governor/check req {} (assoc (op :finalize-spray-application-decision) :confidence 0.99) st)]
      (is (:hard? v))
      (is (some #(= :scope-excluded-action (:rule %)) (:violations v))))))

(deftest hard-on-scope-excluded-op-authorize-spray-application
  (testing "authorizing a spray application is a permanent block, never a routine op"
    (let [st (fresh-store)
          v (governor/check req {} (assoc (op :authorize-spray-application) :confidence 0.99) st)]
      (is (:hard? v))
      (is (some #(= :scope-excluded-action (:rule %)) (:violations v))))))

(deftest hard-on-scope-excluded-op-proceed-with-spray-application
  (testing "proceeding directly with the spray application is a permanent block, never a routine op"
    (let [st (fresh-store)
          v (governor/check req {} (assoc (op :proceed-with-spray-application) :confidence 0.99) st)]
      (is (:hard? v))
      (is (some #(= :scope-excluded-action (:rule %)) (:violations v))))))
...
(deftest hard-on-scope-excluded-rationale-even-with-allowlisted-op
  (testing "defense-in-depth: a rationale that itself attempts to proceed with the spray application is blocked even if the op looks routine"
    (let [st (fresh-store)
          v (governor/check req {} (assoc (op :schedule-crew-operation)
                                           :rationale "recommend we proceed with the spray application now")
                             st)]
      (is (:hard? v))
      (is (some #(= :scope-excluded-action (:rule %)) (:violations v))))))
...
(deftest always-escalates-safety-concern-even-at-high-confidence
  (testing "a fume-exposure/inadequate-ventilation/flammable-material concern always requires human sign-off"
    (let [st (fresh-store)
...
(deftest default-mock-advisor-proposals-never-self-trip-on-scope-exclusion
  (testing "the governor's scope-exclusion term list must never match the mock advisor's own default rationale text for any allowlisted op — CLAUDE.md's known self-tripping bug pattern (rationale legitimately contains bare nouns like 'spray'/'coating'/'safety officer', but never the full finalization-action phrases)"
    (let [st (fresh-store)
          adv (advisor/mock-advisor)
          ops [:log-work-record :schedule-crew-operation
               :flag-safety-concern :coordinate-supply-order]]
      (doseq [o ops]
        (let [request {:sprayer-id "sprayer-1" :op o :shop-id "SH-1"
                        :stake :low :task "routine spray-coating task"
